...
(async () => {
  ...
  for (const item of panelMap) {
    ...
    const svgStr = await page.evaluate(async ({ panelHtml, item }) => {
      ...
      // Set active switch position button to aria-pressed="true" and yellow background (#f7d738)
      if (item.activePos) {
        const switchBtn = panelEl.querySelector(`.program-card-panel-switch-position--${item.activePos}`);
        if (switchBtn) {
          switchBtn.setAttribute('aria-pressed', 'true');
          switchBtn.style.setProperty('background-color', '#f7d738', 'important');
          switchBtn.style.setProperty('background', '#f7d738', 'important');
        }
      }
