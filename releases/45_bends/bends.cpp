...
void __not_in_flash_func(BendsCard::ProcessSample)() {
    ...
    // Synthesise a beat_rising flag: fires exactly once per detected quarter-note beat.
    // With 24 PPQN (OP-1, DIN sync), p1_rising fires 24x per beat — beat_rising fires 1x.
    // With 4 PPQN it fires every 4th pulse, 2 PPQN every 2nd, 1 PPQN every pulse.
    static uint32_t beat_pulse_ctr = 0;
    bool beat_rising = false;
    if (pulse1_live && p1_rising) {
        uint32_t ppqn = 1;
        if (clk_period_samples > 0 && g_clk_period_samples > 0) {
