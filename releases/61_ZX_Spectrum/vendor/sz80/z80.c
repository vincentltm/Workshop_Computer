...
static void in_r_c(z80* const z, uint8_t* r) {
  z->port16 = get_bc(z); // MOD(ZX): full 16-bit port (BC) for the callback
  *r = z->port_in(z, z->c);
  z->zf = *r == 0;
  z->sf = *r >> 7;
  z->pf = parity(*r);
...
static void ini(z80* const z) {
  z->port16 = get_bc(z); // MOD(ZX): full 16-bit port (BC)
...
void exec_opcode(z80* const z, uint8_t opcode) {
  ...
  case 0xDB: {
    ...
    const uint8_t a = z->a;
    z->port16 = (a << 8) | port; // MOD(ZX): full 16-bit port for the callback
  ...
  case 0xD3: {
    const uint8_t port = nextb(z);
    z->port16 = (z->a << 8) | port; // MOD(ZX): full 16-bit port
...
void exec_opcode_ed(z80* const z, uint8_t opcode) {
  ...
  // MOD(ZX): out (c),r — set full 16-bit port (BC) before each callback.
  case 0x41: z->port16 = get_bc(z); z->port_out(z, z->c, z->b); break; // out (c), b
  case 0x49: z->port16 = get_bc(z); z->port_out(z, z->c, z->c); break; // out (c), c
  case 0x51: z->port16 = get_bc(z); z->port_out(z, z->c, z->d); break; // out (c), d
  case 0x59: z->port16 = get_bc(z); z->port_out(z, z->c, z->e); break; // out (c), e
  case 0x61: z->port16 = get_bc(z); z->port_out(z, z->c, z->h); break; // out (c), h
  case 0x69: z->port16 = get_bc(z); z->port_out(z, z->c, z->l); break; // out (c), l
  case 0x71: z->port16 = get_bc(z); z->port_out(z, z->c, 0); break; // out (c), 0
  case 0x79:
    z->port16 = get_bc(z); // MOD(ZX)
